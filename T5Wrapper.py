...
"""
Adaptation of SimpleT5 library (https://github.com/Shivanandroy/simpleT5) 
to expose some parameters for T5. The main changes are:
- Exposing a parameter for num_workers in dataloader and changing it's default value to num_cores/2
...
class T5Wrapper:
    ...
    def from_pretrained(self, model_type="t5", model_name="t5-base", use_gpu=True) -> None:
        """
        loads T5/MT5 Model model for training/finetuning
        Args:
            model_type (str, optional): "t5" or "mt5" . Defaults to "t5".
            model_name (str, optional): exact model architecture name, "t5-base" or "t5-large". Defaults to "t5-base".
            use_gpu (bool, optional): whether to use gpu or not. Defaults to True.
        """
        if model_type == "t5":
            ...
            self.model = T5ForConditionalGeneration.from_pretrained(f"{model_name}", from_flax=True)
            self.tokenizer = T5Tokenizer.from_pretrained(f"{model_name}")
    ...
    def load_model(
        ...
        if model_type == "t5":
            ...
            self.model = T5ForConditionalGeneration.from_pretrained(f"{model_dir}", from_flax=True)
            self.tokenizer = T5Tokenizer.from_pretrained(f"{model_dir}")
    ...
    def predict(
        ...
        Args:
            ...
            labels: List of labels to get the softmax scores.
        Returns:
        ...
        probs = self.get_logit_probabilities(source_text, labels)
        if len(probs) == 1:
            probs = probs[0]
        ...
        return preds, softmax_dic

    def get_logit_probabilities(self, input_text, labels):
        label_ids = get_id_from_labels(labels, self.tokenizer)
        inp_ids = self.tokenizer(input_text, return_tensors="pt", add_special_tokens=True, padding=True)
        decoder_input_ids = torch.tensor([self.tokenizer.pad_token_id]).unsqueeze(0)
        inp_ids.to(self.device)
        decoder_input_ids = decoder_input_ids.to(self.device)
        # print("INP DEVICE: {}, decoder_inp: {}".format(inp_ids.device, decoder_input_ids.device))
        with torch.no_grad():
            logits = self.model(**inp_ids, decoder_input_ids=decoder_input_ids)[0]
        logits = logits.squeeze(1)
        selected_logits = logits[:, label_ids]
    ...
    def predict_batch(
        ...
        generates prediction for T5/MT5 model
        Args:
            source_text (list): List of text for generating predictions
            max_length (int, optional): max token length of prediction. Defaults to 512.
            num_return_sequences (int, optional): number of predictions to be returned. Defaults to 1.
            num_beams (int, optional): number of beams. Defaults to 2.
            top_k (int, optional): Defaults to 50.
            ...
            skip_special_tokens (bool, optional): Defaults to True.
            clean_up_tokenization_spaces (bool, optional): Defaults to True.
            output_scores (bool, optional): Defaults to True.
            return_dict_in_generate(bool, optional): Defaults to True
        Returns:
            list[(str,int)]: returns predictions and their score
        """
        ...
        input_ids = self.tokenizer(
            source_text, return_tensors="pt", add_special_tokens=True, padding=True
        ).input_ids
        input_ids = input_ids.to(self.device)
        generated_ids = self.model.generate(
            input_ids=input_ids,
        ...
        seqs = generated_ids.sequences
        scores = generated_ids.sequences_scores.cpu().data.numpy().tolist()

        preds = self.tokenizer.batch_decode(seqs, skip_special_tokens=skip_special_tokens,
                                            clean_up_tokenization_spaces=clean_up_tokenization_spaces)

        result = []

        assert len(preds) == len(scores), "Length of Predictions and Scores are not same"

        if num_return_sequences > 1:
            preds = to_matrix(preds, num_return_sequences)
            scores = to_matrix(scores, num_return_sequences)
            ...
            result = [(i, j) for i, j in zip(preds, scores)]

        return result


def to_matrix(lst, stride):
    return [lst[i:i + stride] for i in range(0, len(lst), stride)]

def get_id_from_labels(lst: list, tokenizer):
    ids = []
    tokenized = list(tokenizer(lst, return_tensors='np').input_ids)
    for idx, tok in enumerate(tokenized):
        if len(tok) >= 3:
            print("The label {} has multiple tokens, using the first token".format(lst[idx]))
            if tok[0] < 10:
                ids.append(tok[1])
            else:
                ids.append(tok[0])
            ...
            ids.append(tok[0])
    assert len(lst) == len(ids), "Length of the ids is not the same as labels"
    return ids
